...
class WrpSimpleTester : private com::amivoice::wrp::WrpListener {
	public: static void main(const StringList& args) {
		if (args.size() < 4) {
			print("Usage: WrpSimpleTester <url> <audioFileName> <codec> <grammarFileNames> [<authorization>]");
			return;
		}
...

		{
			...
			Pointer<com::amivoice::wrp::WrpListener> listener = new WrpSimpleTester();
